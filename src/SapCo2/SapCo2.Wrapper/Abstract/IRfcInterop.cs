...
namespace SapCo2.Wrapper.Abstract
{
    ...
    {
        RfcResultCodes GetString(IntPtr dataHandle, string name, char[] stringBuffer, uint bufferLength, out uint stringLength, out RfcErrorInfo errorInfo);
        ...
        IntPtr OpenConnection(RfcConnectionParameter[] connectionParams, uint paramCount,
            out RfcErrorInfo errorInfo);

        RfcResultCodes CloseConnection(IntPtr rfcHandle, out RfcErrorInfo errorInfo);

        RfcResultCodes IsConnectionHandleValid(IntPtr rfcHandle, out int isValid,
            out RfcErrorInfo errorInfo);
